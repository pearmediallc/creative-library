-- ============================================
-- MASTER MIGRATION - ALL FEATURES (NO TRANSACTION)
-- ============================================
-- This version runs without a transaction wrapper
-- Each statement executes independently
-- Safe for incremental updates on existing databases
-- Date: 2024-01-13
-- ============================================
...
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS folder_id UUID REFERENCES folders(id) ON DELETE SET NULL;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS upload_batch_id UUID;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS assigned_buyer_id UUID REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS version_number INTEGER DEFAULT 1;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS parent_file_id UUID REFERENCES media_files(id);
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS storage_quota_bytes BIGINT DEFAULT 107374182400;
...
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS metadata_stripped BOOLEAN DEFAULT FALSE;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS metadata_embedded JSONB DEFAULT NULL;
ALTER TABLE media_files ADD COLUMN IF NOT EXISTS metadata_operations TEXT[] DEFAULT '{}';
...
CREATE INDEX IF NOT EXISTS idx_media_files_starred ON media_files(is_starred, starred_at DESC) WHERE is_starred = TRUE AND is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_media_files_user_starred ON media_files(uploaded_by, is_starred, starred_at DESC) WHERE is_starred = TRUE AND is_deleted = FALSE;
...
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS is_public_link BOOLEAN DEFAULT FALSE;
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS link_password VARCHAR(255);
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS link_expires_at TIMESTAMP;
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS disable_download BOOLEAN DEFAULT FALSE;
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS view_count INTEGER DEFAULT 0;
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS last_viewed_at TIMESTAMP;
ALTER TABLE file_permissions ADD COLUMN IF NOT EXISTS max_views INTEGER;
