      ...
      if (isEditor) {
        ...
        const result = await query(
          `SELECT
            ...
            buyer.email as buyer_email,
            creator.name as created_by_name,
            STRING_AGG(DISTINCT e.display_name, ', ') FILTER (WHERE fre.status NOT IN ('reassigned', 'removed')) as assigned_editors,
            COUNT(DISTINCT fre.editor_id) FILTER (WHERE fre.status NOT IN ('reassigned', 'removed')) as total_editors_count,
